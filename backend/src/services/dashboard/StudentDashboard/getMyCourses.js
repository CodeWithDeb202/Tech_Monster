    ...
    return courses
        ...
        .map(item => {
            const remainingTasks = Math.max(
                (item.course.totalTasks || 0) - item.completedTasks,
                0
            );

            const remainingNotes = Math.max(
                (item.course.totalNotes || 0) - item.completedNotes,
                0
            );
            ...
            return {
                ...
                status: item.status,
                certificateEligible: item.course.certificate,
                badgeEligible: item.course.badge,
